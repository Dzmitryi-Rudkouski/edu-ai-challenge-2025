...
export class GameState {
  ...
  getPlayerBoard() {
    ...
    this.#playerShips.forEach(ship => {
      ship.getLocations().forEach(loc => {
        board[loc.row][loc.col] = 'S';
      });
    });

    // Отмечаем ходы CPU
    this.#cpuGuesses.forEach(guessKey => {
      const [row, col] = guessKey.split(',').map(Number);
      const hitShip = this.#findShipAtLocation(this.#playerShips, { row, col });
      board[row][col] = hitShip ? 'X' : 'O';
    });
  ...
  getCPUBoard() {
    ...
    // Размещаем корабли CPU (скрытые)
    this.#cpuShips.forEach(ship => {
      ship.getLocations().forEach(loc => {
        board[loc.row][loc.col] = 'S';
      });
    });

    // Отмечаем ходы игрока
    this.#playerGuesses.forEach(guessKey => {
      const [row, col] = guessKey.split(',').map(Number);
      const hitShip = this.#findShipAtLocation(this.#cpuShips, { row, col });
      board[row][col] = hitShip ? 'X' : 'O';
    });
